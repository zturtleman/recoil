...
static void SetFarClip(void)
{
    ...
    for(i = 0; i < 8; i++)
    {
        ...
        v[0] = tr.viewParms.visBounds[(i & 1)? 0 : 1][0];
        v[1] = tr.viewParms.visBounds[(i & 2)? 0 : 1][1];
        v[2] = tr.viewParms.visBounds[(i & 4)? 0 : 1][2];
...
static qboolean IsMirror(const drawSurf_t *drawSurf, int entityNum)
{
    ...
    for(i = 0 ; i < tr.refdef.num_entities ; i++)
    {
        ...
        // if the entity is just a mirror, don't use as a camera point
        return (e->e.oldorigin[0] == e->e.origin[0] && e->e.oldorigin[1] == e->e.origin[1] && e->e.oldorigin[2] == e->e.origin[2]);
    }
...
static qboolean SurfIsOffscreen(const drawSurf_t *drawSurf, vec4_t clipDest[128])
{
    ...
    return (shortest > (tess.shader->portalRange*tess.shader->portalRange));
}
...
qboolean R_MirrorViewBySurface(drawSurf_t *drawSurf, int entityNum)
{
	...
	//VectorCopy(newParms.pvsOrigin, newParms.ori.origin);
...
void R_SortDrawSurfs(drawSurf_t *drawSurfs, int numDrawSurfs)
{
    ...
    numDrawSurfs = min(MAX_DRAWSURFS, numDrawSurfs);
