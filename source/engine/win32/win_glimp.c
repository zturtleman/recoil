...
static int GLW_ChoosePFD( HDC hDC, PIXELFORMATDESCRIPTOR *pPFD )
{
    ...
    CL_RefPrintf( PRINT_ALL, "...GLW_ChoosePFD( %d, %d, %d )\n", ( int ) pPFD->cColorBits, ( int ) pPFD->cDepthBits, ( int ) pPFD->cStencilBits );
    ...
    if ( maxPFD > MAX_PFDS )
    {
        CL_RefPrintf( PRINT_WARNING, "...numPFDs > MAX_PFDS (%d > %d)\n", maxPFD, MAX_PFDS );
        maxPFD = MAX_PFDS;
    }

    CL_RefPrintf( PRINT_ALL, "...%d PFDs found\n", maxPFD - 1 );
    ...
    for ( i = 1; i <= maxPFD; i++ )
    {
        ...
        if ( ( pfds[i].dwFlags & PFD_GENERIC_FORMAT ) != 0 )
        {
            if ( !r_allowSoftwareGL->integer )
            {
                ...
                {
                    CL_RefPrintf( PRINT_ALL, "...PFD %d rejected, software acceleration\n", i );
                }
                continue;
            }
        }
        ...
        if ( pfds[i].iPixelType != PFD_TYPE_RGBA )
        {
            if ( r_verbose->integer )
            {
                CL_RefPrintf( PRINT_ALL, "...PFD %d rejected, not RGBA\n", i );
            }
            continue;
        }
        ...
        if ( ( ( pfds[i].dwFlags & pPFD->dwFlags ) & pPFD->dwFlags ) != pPFD->dwFlags )
        {
            if ( r_verbose->integer )
            {
                CL_RefPrintf( PRINT_ALL, "...PFD %d rejected, improper flags (%x instead of %x)\n", i, pfds[i].dwFlags, pPFD->dwFlags );
            }
            continue;
        }
    ...
    if ( ( pfds[bestMatch].dwFlags & PFD_GENERIC_FORMAT ) != 0 )
    {
        if ( !r_allowSoftwareGL->integer )
        {
            CL_RefPrintf( PRINT_ALL, "...no hardware acceleration found\n" );
            return 0;
        }
        else
        {
            CL_RefPrintf( PRINT_ALL, "...using software emulation\n" );
        }
    }
    else if ( pfds[bestMatch].dwFlags & PFD_GENERIC_ACCELERATED )
    {
        CL_RefPrintf( PRINT_ALL, "...MCD acceleration found\n" );
    }
    else
    {
        CL_RefPrintf( PRINT_ALL, "...hardware acceleration found\n" );
    }
...
static void GLW_CreatePFD( PIXELFORMATDESCRIPTOR *pPFD, int colorbits, int depthbits, int stencilbits, qboolean stereo )
{
    ...
    if ( stereo )
    {
        CL_RefPrintf( PRINT_ALL, "...attempting to use stereo\n" );
...
static int GLW_MakeContext( PIXELFORMATDESCRIPTOR *pPFD )
{
    ...
    if ( !glw_state.pixelFormatSet )
    {
        ...
        {
            CL_RefPrintf( PRINT_ALL, "...GLW_ChoosePFD failed\n");
            return TRY_PFD_FAIL_SOFT;
        }
        CL_RefPrintf( PRINT_ALL, "...PIXELFORMAT %d selected\n", pixelformat );
        ...
        if ( glConfig.driverType > GLDRV_ICD )
        {
            ...
            {
                CL_RefPrintf ( PRINT_ALL, "...qwglSetPixelFormat failed\n");
                return TRY_PFD_FAIL_SOFT;
            }
        }
        else
        {
            ...
            {
                CL_RefPrintf (PRINT_ALL, "...SetPixelFormat failed\n%i\n", pPFD->cColorBits);
                return TRY_PFD_FAIL_SOFT;
            }
        }
    ...
    if ( !glw_state.hGLRC )
    {
        CL_RefPrintf( PRINT_ALL, "...creating GL context: " );
        if ( ( glw_state.hGLRC = qwglCreateContext( glw_state.hDC ) ) == 0 )
        {
            CL_RefPrintf (PRINT_ALL, "failed\n");

            return TRY_PFD_FAIL_HARD;
        }
        CL_RefPrintf( PRINT_ALL, "succeeded\n" );

        CL_RefPrintf( PRINT_ALL, "...making context current: " );
...
void GLW_InitWGLExtentions(void)
{
    qwglGetExtensionsStringARB = (PFNWGLGETEXTENSIONSSTRINGARBPROC)qwglGetProcAddress("wglGetExtensionsStringARB");

    if(qwglGetExtensionsStringARB)
    {
        char os_extensions_string[BIG_INFO_STRING];

        CL_RefPrintf( PRINT_ALL, "...using WGL_ARB_extensions_strings\n" );
        WGL_ARB_extensions_string_supported = qtrue;

        Q_strncpyz( os_extensions_string, qwglGetExtensionsStringARB(glw_state.hDC), sizeof( os_extensions_string ) );

        if ( strstr( os_extensions_string, "WGL_ARB_pixel_format" ) )
        {
            WGL_ARB_pixel_format_supported = qtrue;
        ...
        {
            CL_RefPrintf( PRINT_ALL, "...WGL_ARB_pixel_format not found\n" );
        }

        if ( strstr( os_extensions_string, "WGL_ARB_pixel_format_float" ) )
        {
            WGL_ARB_pixel_format_float_supported = qtrue;
        }
        else
        {
            CL_RefPrintf( PRINT_ALL, "...WGL_ARB_pixel_format_float not found\n" );
        }

        if ( strstr( os_extensions_string, "WGL_ARB_pbuffer" ) )
        {
            WGL_ARB_pbuffer_supported = qtrue;
        ...
        {
            CL_RefPrintf( PRINT_ALL, "...WGL_ARB_pbuffer not found\n" );
        }

        if ( strstr( os_extensions_string, "WGL_ARB_render_texture" ) )
        {
            WGL_ARB_render_texture_supported = qtrue;
        ...
        {
            CL_RefPrintf( PRINT_ALL, "...WGL_ARB_render_texture not found\n" );
        }
    }
    else
    {
        CL_RefPrintf( PRINT_ALL, "...WGL_ARB_extensions_string not found\n" );
    }
}
...
static qboolean GLW_InitDriver( const char *drivername, int colorbits )
{
    ...
    CL_RefPrintf( PRINT_ALL, "Initializing OpenGL driver\n" );
    ...
    {
        CL_RefPrintf( PRINT_ALL, "...getting DC: " );
    ...
    if ( !glw_state.pixelFormatSet )
    {
        ...
        if ( ( tpfd = GLW_MakeContext( &pfd ) ) != TRY_PFD_SUCCESS )
        {
            ...
            {
                CL_RefPrintf( PRINT_WARNING, "...failed hard\n" );
                return qfalse;
            }
                    ...
                    ( stencilbits == 0 ) )
            {
                ...
                CL_RefPrintf( PRINT_ALL, "...failed to find an appropriate PIXELFORMAT\n" );
        ...
        */
        ...
        {
            CL_RefPrintf( PRINT_ALL, "...failed to select stereo pixel format\n" );
            glConfig.stereoEnabled = qfalse;
        }
    }
...
static qboolean GLW_CreateWindow( const char *drivername, int width, int height, int colorbits, qboolean cdsFullscreen )
{
    ...
    if ( !s_classRegistered )
    {
        ...
        CL_RefPrintf( PRINT_ALL, "...registered window class\n" );
    }
    ...
    if ( !g_wv.hWnd )
    {
        ...
        CL_RefPrintf( PRINT_ALL, "...created window@%d,%d (%dx%d)\n", x, y, w, h );
    }
    else
    {
        CL_RefPrintf( PRINT_ALL, "...window already present, CreateWindowEx skipped\n" );
    }
...
static void PrintCDSError( int value )
{
    ...
    case DISP_CHANGE_RESTART:
        CL_RefPrintf( PRINT_ALL, "restart required\n" );
        break;
    case DISP_CHANGE_BADPARAM:
        CL_RefPrintf( PRINT_ALL, "bad param\n" );
        break;
    case DISP_CHANGE_BADFLAGS:
        CL_RefPrintf( PRINT_ALL, "bad flags\n" );
        break;
    case DISP_CHANGE_FAILED:
        CL_RefPrintf( PRINT_ALL, "DISP_CHANGE_FAILED\n" );
        break;
    case DISP_CHANGE_BADMODE:
        CL_RefPrintf( PRINT_ALL, "bad mode\n" );
        break;
    case DISP_CHANGE_NOTUPDATED:
        CL_RefPrintf( PRINT_ALL, "not updated\n" );
        break;
    default:
        CL_RefPrintf( PRINT_ALL, "unknown error %d\n", value );
        break;
    }
}
    ...
    if ( cdsFullscreen )
    {
        ...
        if ( colorbits != 0 )
        {
            if ( glw_state.allowdisplaydepthchange )
            {
                ...
                CL_RefPrintf( PRINT_ALL, "...using colorsbits of %d\n", colorbits );
            }
            else
            {
                CL_RefPrintf( PRINT_ALL, "WARNING:...changing depth not supported on Win95 < pre-OSR 2.x\n" );
            }
        }
        else
        {
            CL_RefPrintf( PRINT_ALL, "...using desktop display depth of %d\n", glw_state.desktopBitsPixel );
        }
        ...
        {
            CL_RefPrintf( PRINT_ALL, "...already fullscreen, avoiding redundant CDS\n" );

            if ( !GLW_CreateWindow ( drivername, glConfig.vidWidth, glConfig.vidHeight, colorbits, qtrue ) )
            {
                CL_RefPrintf( PRINT_ALL, "...restoring display settings\n" );
        ...
        else
        {
            ...
            else
            {
                ...
                CL_RefPrintf( PRINT_ALL, "...trying next higher resolution:" );
...
static void GLW_InitExtensions( void )
{
    ...
    {
        CL_RefPrintf( PRINT_ALL, "...using WGL_EXT_swap_control\n" );
        r_swapInterval->modified = qtrue;	// force a set next frame
    }
    else
    {
        CL_RefPrintf( PRINT_ALL, "...WGL_EXT_swap_control not found\n" );
    }
    ...
    if ( strstr( glConfig.extensions_string, "WGL_3DFX_gamma_control" ) )
    {
        if ( !r_ignorehwgamma->integer && r_ext_gamma_control->integer )
        {
            ...
            {
                CL_RefPrintf( PRINT_ALL, "...using WGL_3DFX_gamma_control\n" );
            }
            else
            {
                qwglGetDeviceGammaRamp3DFX = NULL;
        ...
        {
            CL_RefPrintf( PRINT_ALL, "...ignoring WGL_3DFX_gamma_control\n" );
        }
    }
    else
    {
        CL_RefPrintf( PRINT_ALL, "...WGL_3DFX_gamma_control not found\n" );
    }
}
...
#define OSR2_BUILD_NUMBER 1111
    ...
    {
        CL_RefPrintf( PRINT_ALL, "GLW_CheckOSVersion() - GetVersionEx failed\n" );
        return qfalse;
    }
...
static qboolean GLW_LoadOpenGL( const char *drivername )
{
    ...
    else
    {
        ...
        CL_RefPrintf( PRINT_ALL, "...assuming '%s' is a standalone driver\n", drivername );
...
void GLimp_Init( void )
{
    ...
    CL_RefPrintf( PRINT_ALL, "Initializing OpenGL subsystem\n" );
...
//	const char *strings[] = { "soft", "hard" };
    ...
    if ( qwglMakeCurrent )
    {
        ...
        CL_RefPrintf( PRINT_ALL, "...wglMakeCurrent( NULL, NULL ): %s\n", success[retVal] );
    }
    ...
    if ( glw_state.hGLRC )
    {
        ...
        CL_RefPrintf( PRINT_ALL, "...deleting GL context: %s\n", success[retVal] );
        glw_state.hGLRC = NULL;
    }
    ...
    if ( glw_state.hDC )
    {
        ...
        CL_RefPrintf( PRINT_ALL, "...releasing DC: %s\n", success[retVal] );
        glw_state.hDC   = NULL;
    }
    ...
    if ( g_wv.hWnd )
    {
        CL_RefPrintf( PRINT_ALL, "...destroying window\n" );
        ShowWindow( g_wv.hWnd, SW_HIDE );
        DestroyWindow( g_wv.hWnd );
        g_wv.hWnd = NULL;
    ...
    if ( glw_state.cdsFullscreen )
    {
        CL_RefPrintf( PRINT_ALL, "...resetting display\n" );
        ChangeDisplaySettings( 0, 0 );
        glw_state.cdsFullscreen = qfalse;
    }
