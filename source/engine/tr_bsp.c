...
===============
*/
static	float R_ColorShiftLightingBytes( byte in[4], byte out[4] )
{
    int		shift, r, g, b;
    ...
    return (float)(r + g + b) * (1.0f/3.0f) * (1.0/256.0);
}

static	float R_ColorShiftLightingBytes2( byte in[4], float out[4] )
{
    int			shift;
    float		r, g, b, max;
    ...
    // shift the data based on overbright range
    r = (in[0] << shift) * (1.0/256.0);
    g = (in[1] << shift) * (1.0/256.0);
    b = (in[2] << shift) * (1.0/256.0);
...
static	void R_LoadVisibility( lump_t *l )
{
    ...
    s_worldData.novis = Hunk_Alloc( len, h_low );
...
#ifdef COLOR_FLOAT
        R_ColorShiftLightingBytes2( verts[i].color, &cv->points[i][7] );
#else
...
static void ParseMesh ( dsurface_t *ds, drawVert_t *verts, msurface_t *surf )
{
    ...
    MAC_STATIC drawVertex_t points[MAX_PATCH_SIZE*MAX_PATCH_SIZE];
    int				lightmapNum;
    vec3_t			bounds[2];
    vec3_t			tmpVec;
    ...
    for ( i = 0 ; i < numPoints ; i++ )
    {
        ...
        R_ColorShiftLightingBytes2( verts[i].color, points[i].color );
    }
...
static void ParseTriSurf( dsurface_t *ds, drawVert_t *verts, msurface_t *surf, int *indexes )
{
    ...
    for ( i = 0 ; i < numVerts ; i++ )
    {
        ...
        R_ColorShiftLightingBytes2( verts[i].color, tri->verts[i].color );
    }
...
void R_MovePatchSurfacesToHunk(void)
{
    ...
    for ( i = 0; i < s_worldData.numsurfaces; i++ )
    {
        ...
        if ( grid->surfaceType != SF_GRID )
            continue;
        //
        size = (grid->width * grid->height - 1) * sizeof( drawVertex_t ) + sizeof( *grid );
        hunkgrid = Hunk_Alloc( size, h_low );
        Com_Memcpy(hunkgrid, grid, size);

        hunkgrid->widthLodError = Hunk_Alloc( grid->width * 4, h_low );
        Com_Memcpy( hunkgrid->widthLodError, grid->widthLodError, grid->width * 4 );

        hunkgrid->heightLodError = Hunk_Alloc( grid->height * 4, h_low );
...
static	void R_LoadFogs( lump_t *l, lump_t *brushesLump, lump_t *sidesLump )
{
    ...
    for ( i=0 ; i<count ; i++, fogs++)
    {
        ...
        out->color[3] = shader->fogParms.color[3] * tr.identityLight;
...
void RE_LoadWorldMap( const char *name )
{
    ...
    startMarker = Hunk_Alloc(0, h_low);
    c_gridVerts = 0;
