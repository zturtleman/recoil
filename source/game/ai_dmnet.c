...
int BotGoForAir(bot_state_t *bs, int tfl, bot_goal_t *ltg, float range)
{
    ...
    if (bs->lastair_time < FloatTime() - 6)
    {
        ...
        {
            botlib_export->ai.BotPushGoal(bs->gs, &goal);
            return qtrue;
        }
        else
        {
            //get a nearby goal outside the water
            while(botlib_export->ai.BotChooseNBGItem(bs->gs, bs->origin, bs->inventory, tfl, ltg, range))
            {
                botlib_export->ai.BotGetTopGoal(bs->gs, &goal);
                //if the goal is not in water
                if (!(botlib_export->aas.AAS_PointContents(goal.origin) & (CONTENTS_WATER|CONTENTS_SLIME|CONTENTS_LAVA)))
                {
                    return qtrue;
                }
                botlib_export->ai.BotPopGoal(bs->gs);
            }
            botlib_export->ai.BotResetAvoidGoals(bs->gs);
        }
    }
    return qfalse;
}
...
int BotNearbyGoal(bot_state_t *bs, int tfl, bot_goal_t *ltg, float range)
{
    ...
    //
    ret = botlib_export->ai.BotChooseNBGItem(bs->gs, bs->origin, bs->inventory, tfl, ltg, range);
    /*
    if (ret)
    {
    	char buf[128];
    	...
    	botlib_export->ai.BotGetTopGoal(bs->gs, &goal);
    	botlib_export->ai.BotGoalName(goal.number, buf, sizeof(buf));
...
int BotReachedGoal(bot_state_t *bs, bot_goal_t *goal)
{
    if (goal->flags & GFL_ITEM)
    {
        ...
        if (botlib_export->ai.BotTouchingGoal(bs->origin, goal))
        {
            if (!(goal->flags & GFL_DROPPED))
            {
                botlib_export->ai.BotSetAvoidGoalTime(bs->gs, goal->number, -1);
            }
            return qtrue;
        }
        //if the goal isn't there
        if (botlib_export->ai.BotItemGoalInVisButNotVisible(bs->entitynum, bs->eye, bs->viewangles, goal))
        {
            /*
            float avoidtime;
            int t;

            avoidtime = botlib_export->ai.BotAvoidGoalTime(bs->gs, goal->number);
            if (avoidtime > 0) {
            	t = botlib_export->aas.AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, goal->areanum, bs->tfl);
        ...
        if (bs->areanum == goal->areanum)
        {
            if (bs->origin[0] > goal->origin[0] + goal->mins[0] && bs->origin[0] < goal->origin[0] + goal->maxs[0])
            {
                ...
                {
                    if (!botlib_export->aas.AAS_Swimming(bs->origin))
                    {
                        return qtrue;
                    }
                }
    ...
    {
        //if touching the goal
        if (botlib_export->ai.BotTouchingGoal(bs->origin, goal)) return qtrue;
        //if the bot got air
...
int BotGetItemLongTermGoal(bot_state_t *bs, int tfl, bot_goal_t *goal)
{
    ...
    if (!botlib_export->ai.BotGetTopGoal(bs->gs, goal))
    {
    ...
    if (bs->ltg_time < FloatTime())
    {
        ...
        if (botlib_export->ai.BotChooseLTGItem(bs->gs, bs->origin, bs->inventory, tfl))
        {
            /*
            char buf[128];
            //get the goal at the top of the stack
        ...
        {//the bot gets sorta stuck with all the avoid timings, shouldn't happen though
            ...
            botlib_export->ai.BotResetAvoidGoals(bs->gs);
            botlib_export->ai.BotResetAvoidReach(bs->ms);
        }
        //get the goal at the top of the stack
        return botlib_export->ai.BotGetTopGoal(bs->gs, goal);
    }
    return qtrue;
}
...
int BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal)
{
    ...
    if (bs->ltgtype == LTG_TEAMHELP && !retreat)
    {
        ...
        if (bs->teammessage_time && bs->teammessage_time < FloatTime())
        {
            ...
            botlib_export->ai.BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
        ...
        if (entinfo.valid)
        {
            ...
            if (areanum && botlib_export->aas.AAS_AreaReachability(areanum))
            {
                //update team goal
    ...
    if (bs->ltgtype == LTG_TEAMACCOMPANY && !retreat)
    {
        ...
        if (bs->teamgoal_time < FloatTime())
        {
            ...
            botlib_export->ai.BotEnterChat(bs->cs, bs->teammate, CHAT_TELL);
            bs->ltgtype = 0;
        }
        //get entity information of the companion
        BotEntityInfo(bs->teammate, &entinfo);
        ...
        if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->teammate))
        {
            ...
            if (VectorLengthSquared(dir) < Square(bs->formation_dist))
            {
                                ...
                                botinfo.origin[1] + botinfo.mins[1] < entinfo.origin[1] + entinfo.maxs[1] + 4)
                        {
                                    ...
                                    botinfo.origin[2] + botinfo.mins[2] < entinfo.origin[2] + entinfo.maxs[2] + 4)
                            {
                                ...
                                if (DotProduct(dir, dir2) > 0.7)
                                {
                                    ...
                                    botlib_export->ai.BotMoveInDirection(bs->ms, dir2, 400, MOVE_WALK);
                                }
                            }
                        }
                    }
                ...
                if (botlib_export->aas.AAS_Swimming(bs->origin)) bs->attackcrouch_time = FloatTime() - 1;
                ...
                if (bs->arrive_time < FloatTime() - 2)
                {
                    ...
                    if (!bs->arrive_time)
                    {
                        botlib_export->ea.EA_Gesture(bs->client);
                    ...
                    {
                        botlib_export->ea.EA_Crouch(bs->client);
                    }
                ...
                {
                    botlib_export->ai.BotResetLastAvoidReach(bs->ms);
                    //get the goal at the top of the stack
                    //botlib_export->ai.BotGetTopGoal(bs->gs, &tmpgoal);
                    //botlib_export->ai.BotGoalName(tmpgoal.number, buf, 144);
            ...
            bs->defendaway_time < FloatTime())
    {
        ...
        if (bs->teammessage_time && bs->teammessage_time < FloatTime())
        {
            botlib_export->ai.BotGoalName(bs->teamgoal.number, buf, sizeof(buf));
    ...
    if (bs->ltgtype == LTG_PATROL && !retreat)
    {
        ...
        //if the bot touches the current goal
        if (botlib_export->ai.BotTouchingGoal(bs->origin, &bs->curpatrolpoint->goal))
        {
    ...
    if (gametype == GT_CTF)
    {
        ...
        if (bs->ltgtype == LTG_RETURNFLAG)
        {
            ...
            //if touching the flag
            if (botlib_export->ai.BotTouchingGoal(bs->origin, goal)) bs->ltgtype = 0;
            //stop after 3 minutes
            if (bs->teamgoal_time < FloatTime())
            {
                bs->ltgtype = 0;
...
void AIEnter_Intermission(bot_state_t *bs, char *s)
{
    ...
    {
        botlib_export->ai.BotEnterChat(bs->cs, 0, bs->chatto);
    }
...
int AINode_Stand(bot_state_t *bs)
{
    ...
    // put up chat icon
    botlib_export->ea.EA_Talk(bs->client);
    // when done standing
...
void AIEnter_Respawn(bot_state_t *bs, char *s)
{
    ...
    //reset some states
    botlib_export->ai.BotResetMoveState(bs->ms);
    botlib_export->ai.BotResetGoalState(bs->gs);
    botlib_export->ai.BotResetAvoidGoals(bs->gs);
    botlib_export->ai.BotResetAvoidReach(bs->ms);
...
int AINode_Respawn(bot_state_t *bs)
{
    ...
    if (bs->respawn_wait)
    {
        ...
        {
            botlib_export->ea.EA_Respawn(bs->client);
        }
    }
    ...
    if (bs->kamikazebody)
    {
        ...
        if ( !(moveresult->flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_MOVEMENTWEAPON)) )
        {
            ...
            if (moveresult->weapon)
            {
                ...
                if (bs->cur_ps.selectedWeapon == moveresult->weapon)
                {
...
int AINode_Seek_ActivateEntity(bot_state_t *bs)
{
    ...
    if (bs->activatestack->shoot)
    {
        ...
        if (bsptrace.fraction >= 1.0 || bsptrace.ent == goal->entitynum)
        {
            targetvisible = qtrue;
            // if holding the right weapon
            if (bs->cur_ps.selectedWeapon == bs->activatestack->weapon)
            {
    ...
    else
    {
        ...
        botlib_export->ai.BotMoveToGoal(&moveresult, bs->ms, goal, bs->tfl);
    ...
    {
        if (botlib_export->ai.BotMovementViewTarget(bs->ms, goal, bs->tfl, 300, target))
        {
            VectorSubtract(target, bs->origin, dir);
            vectoangles(dir, bs->ideal_viewangles);
        }
    ...
    if (BotFindEnemy(bs, -1))
    {
        ...
        else
        {
            ...
            botlib_export->ai.BotEmptyGoalStack(bs->gs);
            //go fight
...
void AIEnter_Seek_NBG(bot_state_t *bs, char *s)
{
    ...
    if (botlib_export->ai.BotGetTopGoal(bs->gs, &goal))
    {
        botlib_export->ai.BotGoalName(goal.number, buf, 144);
...
int AINode_Seek_NBG(bot_state_t *bs)
{
    ...
    bs->enemy = -1;
    //if the bot has no goal
    if (!botlib_export->ai.BotGetTopGoal(bs->gs, &goal)) bs->nbg_time = 0;
    //if the bot touches the current goal
    else if (BotReachedGoal(bs, &goal))
    {
        BotChooseWeapon(bs);
    ...
    {
        if (!botlib_export->ai.BotGetSecondGoal(bs->gs, &goal)) botlib_export->ai.BotGetTopGoal(bs->gs, &goal);
        if (botlib_export->ai.BotMovementViewTarget(bs->ms, &goal, bs->tfl, 300, target))
        {
            VectorSubtract(target, bs->origin, dir);
            vectoangles(dir, bs->ideal_viewangles);
...
int AINode_Battle_Chase(bot_state_t *bs)
{
    ...
    if (botlib_export->ai.BotTouchingGoal(bs->origin, &goal)) bs->chase_time = 0;
