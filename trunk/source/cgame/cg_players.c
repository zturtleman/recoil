...
sfxHandle_t	CG_CustomSound( int clientNum, const char *soundName )
{
    ...
    if ( soundName[0] != '*' )
    {
        return S_RegisterSound( soundName, qfalse );
    }
...
static void CG_LoadClientInfo( clientInfo_t *ci )
{
    ...
    if ( !CG_RegisterClientModelname( ci, ci->modelName, ci->skinName, ci->headModelName, ci->headSkinName, teamname ) )
    {
        ...
        if( cgs.gametype >= GT_TEAM)
        {
            ...
            {
                Com_Error( ERR_DROP,  "DEFAULT_TEAM_MODEL / skin (%s/%s) failed to register", DEFAULT_TEAM_MODEL, ci->skinName );
            }
        }
        else
        {
            ...
            {
                Com_Error( ERR_DROP,  "DEFAULT_MODEL (%s) failed to register", DEFAULT_MODEL );
            }
        }
        modelloaded = qfalse;
    }
    ...
    for ( i = 0 ; i < MAX_CUSTOM_SOUNDS ; i++ )
    {
        ...
        if (modelloaded)
        {
            ci->sounds[i] = S_RegisterSound( va("sound/player/%s/%s", dir, s + 1), qfalse );
        }
        if ( !ci->sounds[i] )
        {
            ci->sounds[i] = S_RegisterSound( va("sound/player/%s/%s", fallback, s + 1), qfalse );
        }
    }
...
void CG_NewClientInfo( int clientNum )
{
    ...
    if ( !CG_ScanForExistingClientInfo( &newInfo ) )
    {
        ...
        forceDefer = Hunk_MemoryRemaining() < 4000000;
...
static void CG_PlayerAngles( centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3] )
{
    ...
    else
    {
        ...
        if ( dir < 0 || dir > 7 )
        {
            Com_Error( ERR_DROP,  "Bad player movement angle" );
        }
    }
...
static qboolean CG_PlayerShadow( centity_t *cent, float *shadowPlane )
{
    ...
    *shadowPlane = 0;
    ...
    CM_BoxTrace( &trace, cent->lerpOrigin, end, mins, maxs, 0, MASK_PLAYERSOLID, qfalse );
