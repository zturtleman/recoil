...
static void CG_ConfigStringModified( void )
{
    ...
    else if ( num >= CS_SOUNDS && num < CS_SOUNDS+MAX_MODELS )
    {
        ...
        {	// player specific sounds don't register here
            cgs.gameSounds[ num-CS_SOUNDS] = S_RegisterSound( str, qfalse );
        }
    }
...
int CG_ParseVoiceChats( const char *filename, voiceChatList_t *voiceChatList, int maxVoiceChats )
{
    ...
    {
        Com_Printf( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i", filename, len, MAX_VOICEFILESIZE ) );
        FS_FCloseFile( f );
        return qfalse;
    }
    ...
    while ( 1 )
    {
        ...
        while(1)
        {
            ...
            if (!Q_stricmp(token, "}"))
                break;
            sound = S_RegisterSound( token, compress );
