...
#define		SV_GentityNum(num) (sv.gentities + (num))
#define		SV_GameClientNum(num) (playerState_t *)((byte *)sv.gameClients + sv.gameClientSize*(num))
#define		SV_SvEntityForGentity(gEnt) ((!gEnt || gEnt->s.number < 0 || gEnt->s.number >= MAX_GENTITIES) ? NULL : &sv.svEntities[gEnt->s.number])
#define		SV_GEntityForSvEntity(svEnt) SV_GentityNum(svEnt - sv.svEntities)
