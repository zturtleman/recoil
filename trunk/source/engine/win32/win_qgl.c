...
{
    CL_RefPrintf( PRINT_ALL, "...shutting down QGL\n" );

    if ( glw_state.hinstOpenGL )
    {
        CL_RefPrintf( PRINT_ALL, "...unloading OpenGL DLL\n" );
...
//	void (__stdcall *grGet)(unsigned int, unsigned int, int*);
    ...
    if ( ( hGlide = LoadLibrary("Glide3X") ) != 0 )
    {
        ...
        else
        {
            ...
            CL_RefPrintf( PRINT_WARNING, "WARNING: could not find grGet in GLIDE3X.DLL\n" );
            numBoards = 0;
        }
        ...
        CL_RefPrintf( PRINT_WARNING, "WARNING: invalid Glide installation!\n" );
#endif
    }
...
qboolean QGL_Init( const char *dllname )
{
    ...
    CL_RefPrintf( PRINT_ALL, "...initializing QGL\n" );
    ...
    if ( strstr( dllname, _3DFX_DRIVER_NAME ) )
    {
        if ( !GlideIsValid() )
        {
            CL_RefPrintf( PRINT_ALL, "...WARNING: missing Glide installation, assuming no 3Dfx available\n" );
            return qfalse;
        }
    }
    ...
    CL_RefPrintf( PRINT_ALL, "...calling LoadLibrary( '%s.dll' ): ", libName );
