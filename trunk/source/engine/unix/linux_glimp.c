  ...
  *key = 0;
...
#ifdef KBD_DBG
  CL_RefPrintf(PRINT_ALL, "XLookupString ret: %d buf: %s keysym: %x\n", XLookupRet, buf, keysym);
#endif
  ...
  default:
    if (XLookupRet == 0)
    {
      ...
      {
        CL_RefPrintf(PRINT_ALL, "Warning: XLookupString failed on KeySym %d\n", keysym);
      }
      return NULL;
    }
    else
...
static void uninstall_grabs(void)
{
  ...
  {
		if (com_developer->value)
			CL_RefPrintf( PRINT_ALL, "DGA Mouse - Disabling DGA DirectVideo\n" );
...
static void GLW_InitGamma()
{
  ...
  if (vidmode_ext)
  {
    if (vidmode_MajorVersion < GAMMA_MINMAJOR || 
      ...
      CL_RefPrintf( PRINT_ALL, "XF86 Gamma extension not supported in this version\n");
      return;
    }
    XF86VidModeGetGamma(dpy, scrnum, &vidmode_InitialGamma);
    CL_RefPrintf( PRINT_ALL, "XF86 Gamma extension initialized\n");
...
static qboolean GLW_LoadOpenGL( const char *name )
{
  ...
  CL_RefPrintf( PRINT_ALL, "...loading %s: ", name );
...
int qXErrorHandler(Display *dpy, XErrorEvent *ev)
{
  ...
  CL_RefPrintf( PRINT_ALL, "X Error of failed request: %s\n", buf);
  CL_RefPrintf( PRINT_ALL, "  Major opcode of failed request: %d\n", ev->request_code, buf);
  CL_RefPrintf( PRINT_ALL, "  Minor opcode of failed request: %d\n", ev->minor_code);  
  CL_RefPrintf( PRINT_ALL, "  Serial number of failed request: %d\n", ev->serial);
  return 0;
}
...
void GLimp_Init( void )
{
  ...
  if ( *r_previousglDriver->string )
  {
    ...
    Cvar_Set( "r_glDriver", r_previousglDriver->string );
  }
  ...
  Cvar_Set( "r_previousglDriver", r_glDriver->string );
...
qboolean GLimp_SpawnRenderThread( void (*function)( void ) )
{
  ...
  glimpRenderThread = function;
	...
	ret = pthread_create( &renderThread,
						  ...
						  NULL );		// argument
	if ( ret ) {
		CL_RefPrintf( PRINT_ALL, "pthread_create returned %d: %s", ret, strerror( ret ) );
    return qfalse;
	} else {
