...
void Com_Init( char *commandLine )
{
    ...
    Cbuf_AddText ("exec config\\default.cfg\n");

    // skip the q3config.cfg if "safe" is on the command line
    if ( !Com_SafeMode() )
    {
        Cbuf_AddText ("exec config\\q3config.cfg\n");
    }

    Cbuf_AddText ("exec config\\autoexec.cfg\n");
...
void Com_WriteConfiguration( void )
{
    ...
    Com_WriteConfigToFile( "config\\q3config.cfg" );
}
