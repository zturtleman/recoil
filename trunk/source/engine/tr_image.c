...
textureMode_t modes[] = {
                            {"GL_NEAREST", F_NEAREST},
                            {"GL_LINEAR", F_LINEAR},
                            {"GL_NEAREST_MIPMAP_NEAREST", F_NEAREST_MIPMAP_NEAREST},
                            {"GL_LINEAR_MIPMAP_NEAREST", F_LINEAR_MIPMAP_NEAREST},
                            {"GL_NEAREST_MIPMAP_LINEAR", F_NEAREST_MIPMAP_LINEAR},
                            {"GL_LINEAR_MIPMAP_LINEAR", F_LINEAR_MIPMAP_LINEAR}
                        };
...
void GL_TextureMode( const char *string )
{
    ...
    gl_filter_min = filterMin[modes[i].filter];
    gl_filter_max = filterMax[modes[i].filter];
...
void R_ImageList_f( void )
{
    ...
    for ( i = 0 ; i < tr.numImages ; i++ )
    {
        ...
        CL_RefPrintf (PRINT_ALL,  "%4i: %4i %4i  %s   %d   ",
...
static void LoadPCX ( const char *filename, byte **pic, byte **palette, int *width, int *height)
{
    ...
    *palette = NULL;
    ...
    if (pcx->manufacturer != 0x0a
            ...
            || ymax >= 1024)
    {
        CL_RefPrintf (PRINT_ALL, "Bad pcx file %s (%i x %i) (%i x %i)\n", filename, xmax+1, ymax+1, pcx->xmax, pcx->ymax);
        return;
    }
...
// FIXME: use bytes_per_line here?
    ...
    if ( raw - (byte *)pcx > len)
    {
        CL_RefPrintf (PRINT_DEVELOPER, "PCX file %s was malformed", filename);
        Z_Free (*pic);
        *pic = NULL;
    }
...
void R_LoadImage( const char *name, byte **pic, int *width, int *height )
{
    ...
    *height = 0;
    ...
    else if(!Q_stricmp(name+len-4, ".tex"))
    {
        LoadTexGenImage(name, pic, width, height);
    }
    ...
    for (image=hashTable[hash]; image; image=image->next)
    {
        if ( !strcmp( name, image->imgName ) )
        {
            ...
            if ( strcmp( name, "*white" ) )
            {
                ...
                {
                    CL_RefPrintf( PRINT_DEVELOPER, "WARNING: reused image %s with mixed mipmap parm\n", name );
                }
                if ( image->allowPicmip != allowPicmip )
                {
                    CL_RefPrintf( PRINT_DEVELOPER, "WARNING: reused image %s with mixed allowPicmip parm\n", name );
                }
                if ( image->wrapClampMode != glWrapClampMode )
                {
                    CL_RefPrintf( PRINT_ALL, "WARNING: reused image %s with mixed glWrapClampMode parm\n", name );
                }
            }
            return image;
        }
...
void R_CreateBuiltinImages( void )
{
    ...
    for (x=0 ; x<DEFAULT_SIZE ; x++)
    {
        for (y=0 ; y<DEFAULT_SIZE ; y++)
        {
            data[y][x][0] =
                data[y][x][1] =
                    data[y][x][2] = tr.identityLight * 255;
            data[y][x][3] = 255;
        }
    }
...
void R_DeleteTextures( void )
{
    ...
    for ( i=0; i<tr.numImages ; i++ )
    {
        switch(tr.images[i]->target)
        {
        case GL_TEXTURE_2D:
            qglDeleteTextures( 1, &tr.images[i]->texnum );
            break;
        case GL_RENDERBUFFER_EXT:
            qglDeleteRenderbuffersEXT(1, &tr.images[i]->texnum );
            break;
        default:
            break;
        }
