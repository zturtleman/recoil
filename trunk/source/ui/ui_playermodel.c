...
{
    Cvar_Set( "model", s_playermodel.modelskin );
    Cvar_Set( "headmodel", s_playermodel.modelskin );
    Cvar_Set( "team_model", s_playermodel.modelskin );
    Cvar_Set( "team_headmodel", s_playermodel.modelskin );
}
...
static void PlayerModel_PicEvent( void* ptr, int event )
{
    ...
    if (pdest)
    {
        ...
        if( Hunk_MemoryRemaining() > LOW_MEMORY )
        {
            PlayerModel_UpdateModel();
        }
    }
...
static void PlayerModel_BuildList( void )
{
    ...
    for (i=0; i<numdirs && s_playermodel.nummodels < MAX_PLAYERMODELS; i++,dirptr+=dirlen+1)
    {
        ...
        for (j=0; j<numfiles && s_playermodel.nummodels < MAX_PLAYERMODELS;j++,fileptr+=filelen+1)
        {
            ...
            if( precache )
            {
                S_RegisterSound( va( "sound/player/announce/%s_wins.wav", skinname), qfalse );
            }
        }
    }
...
static void PlayerModel_SetMenuItems( void )
{
    ...
    // name
    Cvar_VariableStringBuffer( "name", s_playermodel.playername.string, 16 );
    Q_CleanStr( s_playermodel.playername.string );

    // model
    Cvar_VariableStringBuffer( "model", s_playermodel.modelskin, 64 );
...
void PlayerModel_Cache( void )
{
    ...
    {
        RE_RegisterShaderNoMip( playermodel_artlist[i] );
    }

    PlayerModel_BuildList();
    for( i = 0; i < s_playermodel.nummodels; i++ )
    {
        RE_RegisterShaderNoMip( s_playermodel.modelnames[i] );
    }
}
