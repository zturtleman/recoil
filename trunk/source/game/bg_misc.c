...
void BG_PlayerStateToEntityStateExtraPolate( playerState_t *ps, entityState_t *s, int time, qboolean snap )
{
    ...
    s->eType = (ps->pm_type == PM_INTERMISSION || ps->pm_type == PM_SPECTATOR || ps->stats[STAT_HEALTH] <= GIB_HEALTH) ? ET_INVISIBLE : ET_PLAYER;
...
typedef struct
{
    ...
    // both
    int (*bgGetAttackTime)(int weapon, qboolean a, qboolean d, qboolean alt);
...
{
    return GetChar(number)->bgGetAttackTime(weapon, a, d, alt);
}

// SERVER
void svFireWeapon(gentity_t *ent, qboolean alt)
{
    GetChar(ent->s.number)->svFireWeapon(ent, alt);
}
...
{
    return GetChar(ent->currentState.clientNum)->clAmmoIcon(ent->currentState.weapon);
}
