...
void QDECL BotAI_BotInitialChat( bot_state_t *bs, char *type, ... )
{
    ...
    botlib_export->ai.BotInitialChat( bs->cs, type, mcontext, vars[0], vars[1], vars[2], vars[3], vars[4], vars[5], vars[6], vars[7] );
}
...
void BotInterbreedBots(void)
{
    ...
    if (botlib_export->ai.GeneticParentsAndChildSelection(MAX_CLIENTS, ranks, &parent1, &parent2, &child))
    {
        botlib_export->ai.BotInterbreedGoalFuzzyLogic(botstates[parent1]->gs, botstates[parent2]->gs, botstates[child]->gs);
        botlib_export->ai.BotMutateGoalFuzzyLogic(botstates[child]->gs, 1);
    }
...
void BotWriteInterbreeded(char *filename)
{
    ...
    if (bestbot >= 0)
    {
        ...
        botlib_export->ai.BotSaveGoalFuzzyLogic(botstates[bestbot]->gs, filename);
    }
}
    ...
    while( SV_BotGetConsoleMessage(client, buf, sizeof(buf)) )
    {
        ...
        *args++ = '\0';
        ...
        else if (!Q_stricmp(buf, "print"))
        {
            ...
            botlib_export->ai.BotQueueConsoleMessage(bs->cs, CMS_NORMAL, args);
        }
        else if (!Q_stricmp(buf, "chat"))
        {
            ...
            memmove(args, args+1, strlen(args));
            args[strlen(args)-1] = '\0';
            botlib_export->ai.BotQueueConsoleMessage(bs->cs, CMS_CHAT, args);
        }
...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart)
{
    ...
    if (!botlib_export->aas.AAS_Initialized())
    {
    ...
    bs->character = botlib_export->ai.BotLoadCharacter(settings->characterfile, settings->skill);
    if (!bs->character)
    {
    ...
    bs->gs = botlib_export->ai.BotAllocGoalState(client);
    //load the item weights
    botlib_export->ai.Characteristic_String(bs->character, CHARACTERISTIC_ITEMWEIGHTS, filename, MAX_PATH);
    errnum = botlib_export->ai.BotLoadItemWeights(bs->gs, filename);
    ...
    bs->ws = botlib_export->ai.BotAllocWeaponState();
    //load the weapon weights
    botlib_export->ai.Characteristic_String(bs->character, CHARACTERISTIC_WEAPONWEIGHTS, filename, MAX_PATH);
    errnum = botlib_export->ai.BotLoadWeaponWeights(bs->ws, filename);
    if (errnum != BLERR_NOERROR)
    {
        botlib_export->ai.BotFreeGoalState(bs->gs);
    ...
    bs->cs = botlib_export->ai.BotAllocChatState();
    //load the chat file
    botlib_export->ai.Characteristic_String(bs->character, CHARACTERISTIC_CHAT_FILE, filename, MAX_PATH);
    botlib_export->ai.Characteristic_String(bs->character, CHARACTERISTIC_CHAT_NAME, name, MAX_PATH);
    errnum = botlib_export->ai.BotLoadChatFile(bs->cs, filename, name);
    if (errnum != BLERR_NOERROR)
    {
    ...
    botlib_export->ai.Characteristic_String(bs->character, CHARACTERISTIC_GENDER, gender, MAX_PATH);
    //set the chat gender
    if (*gender == 'f' || *gender == 'F') botlib_export->ai.BotSetChatGender(bs->cs, CHAT_GENDERFEMALE);
    else if (*gender == 'm' || *gender == 'M') botlib_export->ai.BotSetChatGender(bs->cs, CHAT_GENDERMALE);
    ...
    bs->ms = botlib_export->ai.BotAllocMoveState();
    bs->walker = botlib_export->ai.Characteristic_BFloat(bs->character, CHARACTERISTIC_WALKER, 0, 1);
    numbots++;

    if (Cvar_VariableIntegerValue("bot_testichat"))
    {
        botlib_export->BotLibVarSet("bot_testichat", "1");
        BotChatTest(bs);
    }
    ...
    if (bot_interbreed)
    {
        botlib_export->ai.BotMutateGoalFuzzyLogic(bs->gs, 1);
    }
...
void BotResetState(bot_state_t *bs)
{
    ...
    if (bs->ms) botlib_export->ai.BotResetMoveState(bs->ms);
    if (bs->gs) botlib_export->ai.BotResetGoalState(bs->gs);
    if (bs->ws) botlib_export->ai.BotResetWeaponState(bs->ws);
    if (bs->gs) botlib_export->ai.BotResetAvoidGoals(bs->gs);
    if (bs->ms) botlib_export->ai.BotResetAvoidReach(bs->ms);
}
...
int BotAIStartFrame(int time)
{
    ...
    {
        botlib_export->BotLibVarSet("memorydump", "1");
        Cvar_Set("bot_memorydump", "0");
    }
    if (bot_saveroutingcache->integer)
    {
        botlib_export->BotLibVarSet("saveroutingcache", "1");
    ...
    if ( botlib_residual >= thinktime )
    {
        ...
        botlib_export->BotLibStartFrame((float) time / 1000);

        if (!botlib_export->aas.AAS_Initialized()) return qfalse;
        ...
        for (i = 0; i < MAX_GENTITIES; i++)
        {
            ...
            if (!ent->inuse)
            {
                botlib_export->BotLibUpdateEntity(i, NULL);
                continue;
            }
            if (!ent->r.linked)
            {
            ...
            if (ent->s.eType == ET_MISSILE && ent->s.weapon != WP_MELEE)
            {
                botlib_export->BotLibUpdateEntity(i, NULL);
                continue;
            }
    ...
    floattime = botlib_export->aas.AAS_Time();
...
int BotAISetup( int restart )
{
    ...
    bot_thinktime = Cvar_Get("bot_thinktime", "100", CVAR_CHEAT);
    bot_memorydump = Cvar_Get("bot_memorydump", "0", CVAR_CHEAT);
    bot_saveroutingcache = Cvar_Get("bot_saveroutingcache", "0", CVAR_CHEAT);
    bot_pause = Cvar_Get("bot_pause", "0", CVAR_CHEAT);
    bot_report = Cvar_Get("bot_report", "0", CVAR_CHEAT);
    bot_testsolid = Cvar_Get("bot_testsolid", "0", CVAR_CHEAT);
    bot_testclusters = Cvar_Get("bot_testclusters", "0", CVAR_CHEAT);
    bot_developer = Cvar_Get("bot_developer", "0", CVAR_CHEAT);
    bot_interbreedchar = Cvar_Get("bot_interbreedchar", "", 0);
    bot_interbreedbots = Cvar_Get("bot_interbreedbots", "10", 0);
    bot_interbreedcycle = Cvar_Get("bot_interbreedcycle", "20", 0);
    bot_interbreedwrite = Cvar_Get("bot_interbreedwrite", "", 0);
