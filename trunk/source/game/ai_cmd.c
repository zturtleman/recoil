...
int BotGetItemTeamGoal(char *goalname, bot_goal_t *goal)
{
    ...
    do
    {
        i = botlib_export->ai.BotGetLevelItemGoal(i, goalname, goal);
        if (i > 0)
        {
...
float BotGetTime(bot_match_t *match)
{
    ...
    {
        //get the time string
        botlib_export->ai.BotMatchVariable(match, TIME, timestring, MAX_MESSAGE_SIZE);
        //match it to find out if the time is in seconds or minutes
        if (botlib_export->ai.BotFindMatch(timestring, &timematch, MTCONTEXT_TIME))
        {
            ...
            {
                botlib_export->ai.BotMatchVariable(&timematch, TIME, timestring, MAX_MESSAGE_SIZE);
...
int BotGetPatrolWaypoints(bot_state_t *bs, bot_match_t *match)
{
    ...
    patrolflags = 0;
    //
    botlib_export->ai.BotMatchVariable(match, KEYAREA, keyarea, MAX_MESSAGE_SIZE);
    //
    while(1)
    {
        if (!botlib_export->ai.BotFindMatch(keyarea, &keyareamatch, MTCONTEXT_PATROLKEYAREA))
        {
            botlib_export->ea.EA_SayTeam(bs->client, "what do you say?");
            BotFreeWaypoints(newpatrolpoints);
            bs->patrolpoints = NULL;
            return qfalse;
        }
        botlib_export->ai.BotMatchVariable(&keyareamatch, KEYAREA, keyarea, MAX_MESSAGE_SIZE);
        ...
        {
            botlib_export->ai.BotMatchVariable(&keyareamatch, MORE, keyarea, MAX_MESSAGE_SIZE);
        }
        else
        {
            break;
    ...
    {
        botlib_export->ea.EA_SayTeam(bs->client, "I need more key points to patrol\n");
        BotFreeWaypoints(newpatrolpoints);
        newpatrolpoints = NULL;
        return qfalse;
    }
...
int BotAddressedToBot(bot_state_t *bs, bot_match_t *match)
{
    ...
    {
        botlib_export->ai.BotMatchVariable(match, ADDRESSEE, addressedto, sizeof(addressedto));
        //the name of this bot
        ClientName(bs->client, botname, 128);
        //
        while(botlib_export->ai.BotFindMatch(addressedto, &addresseematch, MTCONTEXT_ADDRESSEE))
        {
            ...
            {
                botlib_export->ai.BotMatchVariable(&addresseematch, TEAMMATE, name, sizeof(name));
                if (strlen(name))
                {
                    if (stristr(botname, name)) return qtrue;
                    if (stristr(bs->subteam, name)) return qtrue;
                }
                botlib_export->ai.BotMatchVariable(&addresseematch, MORE, addressedto, MAX_MESSAGE_SIZE);
            }
            else
            {
                botlib_export->ai.BotMatchVariable(&addresseematch, TEAMMATE, name, MAX_MESSAGE_SIZE);
                if (strlen(name))
                {
                    if (stristr(botname, name)) return qtrue;
                    if (stristr(bs->subteam, name)) return qtrue;
    ...
    else
    {
        ...
        if (!botlib_export->ai.BotFindMatch(match->string, &tellmatch, MTCONTEXT_REPLYCHAT) ||
...
void BotMatch_HelpAccompany(bot_state_t *bs, bot_match_t *match)
{
    ...
    botlib_export->ai.BotMatchVariable(match, TEAMMATE, teammate, sizeof(teammate));
    //get the client to help
    if (botlib_export->ai.BotFindMatch(teammate, &teammatematch, MTCONTEXT_TEAMMATE) &&
    ...
    if (client < 0)
    {
        ...
        client = ClientFromName(netname);
        botlib_export->ai.BotEnterChat(bs->cs, client, CHAT_TELL);
        return;
    }
    ...
    if (entinfo.valid)
    {
        ...
        if (areanum)
        {// && botlib_export->aas.AAS_AreaReachability(areanum)) {
    ...
    if (bs->teamgoal.entitynum < 0)
    {
        ...
        if (match->subtype & ST_NEARITEM)
        {
            ...
            botlib_export->ai.BotMatchVariable(match, ITEM, itemname, sizeof(itemname));
            //
            if (!BotGetMessageTeamGoal(bs, itemname, &bs->teamgoal))
            {
                //BotAI_BotInitialChat(bs, "cannotfind", itemname, NULL);
                //botlib_export->ai.BotEnterChat(bs->cs, bs->client, CHAT_TEAM);
                return;
            }
        }
    }
    ...
    if (bs->teamgoal.entitynum < 0)
    {
        ...
        client = ClientFromName(netname);
        botlib_export->ai.BotEnterChat(bs->cs, client, CHAT_TEAM);
        return;
    }
    //the team mate
    bs->teammate = client;
...
void BotMatch_DefendKeyArea(bot_state_t *bs, bot_match_t *match)
{
    ...
    if (!BotAddressedToBot(bs, match)) return;
    //get the match variable
    botlib_export->ai.BotMatchVariable(match, KEYAREA, itemname, sizeof(itemname));
    //
    if (!BotGetMessageTeamGoal(bs, itemname, &bs->teamgoal))
    {
        //BotAI_BotInitialChat(bs, "cannotfind", itemname, NULL);
...
void BotMatch_TaskPreference(bot_state_t *bs, bot_match_t *match)
{
    ...
    botlib_export->ai.BotMatchVariable(match, NETNAME, teammatename, sizeof(teammatename));
    ...
    botlib_export->ai.BotEnterChat(bs->cs, teammate, CHAT_TELL);
    BotVoiceChatOnly(bs, teammate, VOICECHAT_YES);
    botlib_export->ea.EA_Action(bs->client, ACTION_AFFIRMATIVE);
}
...
void BotMatch_JoinSubteam(bot_state_t *bs, bot_match_t *match)
{
    ...
    botlib_export->ai.BotMatchVariable(match, TEAMNAME, teammate, sizeof(teammate));
...
void BotMatch_CheckPoint(bot_state_t *bs, bot_match_t *match)
{
    ...
    if (!TeamPlayIsOn()) return;
    //
    botlib_export->ai.BotMatchVariable(match, POSITION, buf, MAX_MESSAGE_SIZE);
    ...
    //
    botlib_export->ai.BotMatchVariable(match, NAME, buf, MAX_MESSAGE_SIZE);
...
void BotMatch_FormationSpace(bot_state_t *bs, bot_match_t *match)
{
    ...
    if (!BotAddressedToBot(bs, match)) return;
    //
    botlib_export->ai.BotMatchVariable(match, NUMBER, buf, MAX_MESSAGE_SIZE);
...
void BotMatch_Suicide(bot_state_t *bs, bot_match_t *match)
{
    ...
    if (!BotAddressedToBot(bs, match)) return;
    //
    botlib_export->ea.EA_Command(bs->client, "kill");
    //
    botlib_export->ai.BotMatchVariable(match, NETNAME, netname, sizeof(netname));
    client = ClientFromName(netname);
    //
...
void BotMatch_StartTeamLeaderShip(bot_state_t *bs, bot_match_t *match)
{
    ...
    if (match->subtype & ST_I)
    {
        ...
        botlib_export->ai.BotMatchVariable(match, NETNAME, teammate, sizeof(teammate));
...
void BotMatch_WhoIsTeamLeader(bot_state_t *bs, bot_match_t *match)
{
    ...
    {
        botlib_export->ea.EA_SayTeam(bs->client, "I'm the team leader\n");
    }
}
...
float BotNearestVisibleItem(bot_state_t *bs, char *itemname, bot_goal_t *goal)
{
    ...
    do
    {
        i = botlib_export->ai.BotGetLevelItemGoal(i, itemname, &tmpgoal);
        botlib_export->ai.BotGoalName(tmpgoal.number, name, sizeof(name));
...
void BotMatch_WhereAreYou(bot_state_t *bs, bot_match_t *match)
{
    ...
    if (bestitem != -1)
    {
        ...
        {
            redtt = botlib_export->aas.AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, ctf_redflag.areanum, TFL_DEFAULT);
            bluett = botlib_export->aas.AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, ctf_blueflag.areanum, TFL_DEFAULT);
...
void BotMatch_Kill(bot_state_t *bs, bot_match_t *match)
{
    ...
    botlib_export->ai.BotMatchVariable(match, ENEMY, enemy, sizeof(enemy));
    //
...
void BotMatch_CTF(bot_state_t *bs, bot_match_t *match)
{
    ...
    if (gametype == GT_CTF)
    {
        botlib_export->ai.BotMatchVariable(match, FLAG, flag, sizeof(flag));
